...
func (uq *UserQuery) QueryLocations() *LocationQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLocations: uq.withLocations.Clone(),
...
func (uq *UserQuery) WithLocations(opts ...func(*LocationQuery)) *UserQuery {
	query := (&LocationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLocations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLocations; query != nil {
		if err := uq.loadLocations(ctx, query, nodes,
...
func (uq *UserQuery) loadLocations(ctx context.Context, query *LocationQuery, nodes []*User, init func(*User), assign func(*User, *Location)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.LocationsTable)
		s.Join(joinT).On(s.C(location.FieldID), joinT.C(user.LocationsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.LocationsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.LocationsPrimaryKey[1]))
