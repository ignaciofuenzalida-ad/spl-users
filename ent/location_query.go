...
func (lq *LocationQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Location, init func(*Location), assign func(*Location, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(location.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(location.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(location.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(location.UsersPrimaryKey[0]))
